...
public class GestionGroupe {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					GestionGroupe window = new GestionGroupe();
	...
	private void initialize() {
		 ...
		 */
		ArrayList<Groupe> liste = groupeDAO.getList();
		String colonne[] = {"Id", "Capacite Maximum", "Filiere"};
		
		if(this.donnee == null) {
			donnee = getTableauGroupe(liste, colonne);
		}
		...
		table.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if(modificationActive) {
					...
					ModifierGroupe modifierGroupe = new ModifierGroupe(id);
					modifierGroupe.getFrame().setVisible(true);
				
				}else if(suppressionActive) {
					
					int id = Integer.parseInt((String)table.getValueAt(row, 0));
					groupeDAO.delete(id);
					ArrayList<Groupe> listeGroupe = groupeDAO.getList();
					donnee = getTableauGroupe(listeGroupe, colonne);
		...
		JLabel lblListeDesGroupes = new JLabel("Liste des Groupes");
	 ...
	 * @return data
	 */
	public String[][] getTableauGroupe(ArrayList<Groupe> listeGroupe, String[] colonne){
		String data[][] = new String[listeGroupe.size()][colonne.length];
		for(int i=0; i<listeGroupe.size(); i++) {
			data[i][0] = String.valueOf(listeGroupe.get(i).getNumeroGroupe());
			data[i][1] = String.valueOf(listeGroupe.get(i).getCapaciteMax());
			data[i][2] = listeGroupe.get(i).getFiliere();			
		}
		return data;
	}
